...
author: "Medard Pikul, Hanna Móżdżyńska, Zofia Stasiak, Marta Rutkowska" 
date: "`r Sys.Date()`"
output:
...
### Średni czas używania Social Mediów
...
## Charakterystyka wpływu mediów społecznościowych

### Poziom uzależnienia od Social Mediów
...
## Statystyki opisowe średniego dziennego czasu korzystania z mediów społecznościowych w podziale na 5 najczęściej używanych platform
...
## Statystyki opisowe poziomu uzależnienia od mediów społecznościowych
```{r}
...
## Macierz korelacji między poziomem uzależnienienia od social mediów, zdrowiem psychicznym, wiekiem i średnim czasem używania social mediów
...
## Czy istnieje zależność między średnim czasem używania social mediów a liczbą konfliktów?
```{r}
...
## Analiza ANOVA wieloczynnikowa - wpływ snu, czasu korzystania z telefonu i poziomu uzależnienia na wyniki akademickie
```{r}
dane %>%
  mutate(
