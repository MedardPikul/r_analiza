...
```{r}
#zmienic na zmienna binarna (czy wplywa na wyniki akademickie)
dane %>%
  anova_test(Addicted_Score ~ Conflicts_Over_Social_Media*Avg_Daily_Usage_Hours)%>%
  kbl()%>%
  kable_styling(bootstrap_options = c("striped","hover"))
#tutaj zmienic te dane jesli chcemy to zawrzeć, ja dalem raczej losowe
#wykres interakcji conflicts vs average hours
interaction.plot(
  x.factor=dane$Conflicts_Over_Social_Media,
  trace.factor = dane$Avg_Daily_Usage_Hours,
  response = dane$Addicted_Score,
  fun=mean,
  type = "b",
  col=c("red","blue"),
  pch=c(16,18),
  xlab="liczba konfliktów",
  ylab="średni addicted score",
  legend=TRUE,
)
